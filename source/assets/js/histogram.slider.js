; (function ($, window, document, undefined) {
    ...
    var Plugin = function (element, options) {
        ...
        this.element.setValue= this.setValue;
        this.init(options);
    };
        ...
        init: function (options) {
            ...
            $("#" + sliderName).slider({
                ...
                slide: function (event, ui) {
                    $(self.element).trigger('update', [ui.values]);
            ...
            $(self.element).trigger('update', [self.options.selectedRange]);
            self.element[0].setValue = function(values) {
                $("#" + sliderName).slider( "values", values);
                $(self.element).trigger('update', [values]);
                updateHistogram(values, self.options.sliderRange[0], rangePerBin, histogramName, sliderName);
            }
            self.element[0].setOptions = function(options) {
                $("#" + sliderName).slider("option", options);

                var max = $("#" + sliderName).slider( "option", "max");
                var min = $("#" + sliderName).slider( "option", "min");

                $("#" + sliderName).slider( "values", [min, max]);
                $(self.element).trigger('update', [[min, max]]);

                updateHistogram([min, max], self.options.sliderRange[0], rangePerBin, histogramName, sliderName);
            }
        },
    };
